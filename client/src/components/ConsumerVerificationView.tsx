...
export const ConsumerVerificationView: React.FC<ConsumerVerificationViewProps> = ({ batchId = 'HC-TG-2026-001', onNavigate }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-16">
      ...
      <header className="bg-white border-b border-slate-200/80 sticky top-0 z-40 px-3 sm:px-6 py-2.5 sm:py-3 shadow-2xs">
        <div className="max-w-3xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
          <div className="flex items-center space-x-2 min-w-0">
            {onNavigate && (
              <button
                onClick={() => onNavigate('dashboard')}
                className="px-2.5 sm:px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold text-xs rounded-xl flex items-center space-x-1 transition-colors shrink-0"
            ...
            <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-xl bg-gradient-to-tr from-amber-500 to-amber-400 flex items-center justify-center text-slate-950 font-black shadow-2xs shrink-0">
              <ShieldCheck className="h-4 w-4 sm:h-5 sm:w-5 text-slate-950" />
            </div>
            <div className="min-w-0 truncate">
              <span className="font-extrabold text-sm sm:text-lg text-slate-900 tracking-tight block leading-none truncate">
                HONEY CHAIN
              </span>
              <span className="text-[9px] sm:text-[10px] text-amber-800 font-bold uppercase tracking-wider block truncate">
                VERIFIED HONEY PASSPORT
              </span>
            </div>
          </div>

          <form onSubmit={handleSearchSubmit} className="relative w-32 sm:w-56 shrink-0">
            ...
            <input
              ...
              className="w-full pl-7 sm:pl-8 pr-2.5 py-1.5 bg-slate-100 border border-slate-200 rounded-xl text-xs font-mono font-bold focus:bg-white focus:outline-none focus:ring-2 focus:ring-amber-500/50"
            />
          </form>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-3 sm:px-4 pt-4 sm:pt-6 space-y-4 sm:space-y-6">
        ...
        {!loading && data && (
          <div className="space-y-6">
            ...
            <div className={`p-4 sm:p-8 rounded-2xl sm:rounded-3xl border shadow-sm space-y-3 text-center sm:text-left flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6 ${
              data.authentic 
